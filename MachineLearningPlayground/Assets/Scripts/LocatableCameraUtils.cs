...
public static class LocatableCameraUtils
{
    ...
    public static Vector2 WorldCoordToPixelCoord(Matrix4x4 cameraToWorldMatrix, Matrix4x4 projectionMatrix, HoloLensCameraStream.Resolution cameraResolution, Vector3 wordlCoordinates)
    {
        // Transform the world coordinates into clip space
        Vector4 clipSpacePosition = projectionMatrix * cameraToWorldMatrix.inverse * new Vector4(wordlCoordinates.x, wordlCoordinates.y, wordlCoordinates.z, 1);

        // Divide the clip space position by its w component to get normalized device coordinates (NDC)
        Vector3 ndc = clipSpacePosition / clipSpacePosition.w;

        // Map the NDC coordinates to pixel coordinates based on the camera resolution
        Vector2 pixelCoordinates = new Vector2(
            (ndc.x + 1) * 0.5f * cameraResolution.width,
            (1 - ndc.y) * 0.5f * cameraResolution.height);
